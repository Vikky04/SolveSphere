...
function LinkedList() {
    // Filter the topics as per the position (4 in this case)
    const filteredData = Data.filter((topic) => topic.position === 4);
    ...
    // Ensure that there is at least one topic in the filteredData to access.
    const topic = filteredData[0] || {}; // Default to an empty object if no topic is found

    return (
        <React.Fragment>
            <Navbar />
            <div className="abtusheading">{topic.topic || 'Linked List'}</div> {/* Safe access to the topic */}
